...
/**
 * Class related to downloading a image and creating a layout according to the retrieved recommended product information
 * @author      Seontaek Oh
 * @version     1.0
 * @since       1.0
 */
public class DownloadRecommendedProductImageTask extends AsyncTask<String, Void, Bitmap> {
    ...
    /**
     * Constructor of the class related to downloading a image and creating a layout according to the retrieved recommended product information
     * @param llProductList
     * @param product
     * @param ctx
     * @param fm
     */
    public DownloadRecommendedProductImageTask(LinearLayout llProductList, Product product, Context ctx, FragmentManager fm) {
        this.llProductList = llProductList;
        this.layoutParams = new LayoutParams(200, 200);
    ...
    protected void onPostExecute(Bitmap result) {
        ...
        if(title.length() > maxLength){
            title = title.substring(0,maxLength-2)+"..";
        }
        //Use hint as a variable storage.
        tvProductName.setHint(String.valueOf(product.getId()));
        tvProductName.setText(Html.fromHtml("<u>" + title + "</u>"));
